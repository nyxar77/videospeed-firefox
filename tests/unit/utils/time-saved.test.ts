...
describe('saved time calculations', () => {
  it('counts only the wall-clock time avoided by watched media', () => {
    expect(calculateSavedMilliseconds(10, 5_000)).toBe(5_000);
    expect(calculateSavedMilliseconds(10, 10_000)).toBe(0);
    expect(calculateSavedMilliseconds(5, 10_000)).toBe(0);
  });

  it('rejects seek jumps and invalid segments', () => {
    expect(calculateSavedMilliseconds(600, 1_000)).toBe(0);
    expect(calculateSavedMilliseconds(-1, 1_000)).toBe(0);
    expect(calculateSavedMilliseconds(1, 0)).toBe(0);
  });
